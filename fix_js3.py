...
# We want to insert 
ew_osint_js right before lucide.createIcons();\n      } catch (err) {
# Let's search for it cleanly:
target = "lucide.createIcons();"

parts = html.split(target)
if len(parts) >= 2:
    # There are multiple lucide.createIcons(). We want the one in runEBAssessment!
    # The one in runEBAssessment is before } catch (err) {
    for i, part in enumerate(parts):
        # part is the string BEFORE the target. parts[i+1] is the string AFTER the target.
