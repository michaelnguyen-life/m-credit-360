...
# We will inject cross-sell update logic at the end of runEBAssessment
injection = """
            ...
            try {
                ...
                if (csContainer) {
                    ...
                    <div class="p-2.5 rounded-lg bg-orange-50 border border-orange-100 relative overflow-hidden group hover:shadow-md transition">
                      ...
                      <p class="text-[10px] text-[#667085] leading-tight">Tối ưu dòng tiền phái thu</p>
...
# Inject before document.getElementById('eb-results-container').classList.remove('hidden');
content = content.replace("document.getElementById('eb-results-container').classList.remove('hidden');", injection + "\n            document.getElementById('eb-results-container').classList.remove('hidden');")
