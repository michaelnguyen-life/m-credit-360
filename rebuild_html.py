...
# Look for the end of the cross sell panel in the original file
# The original HTML structure ends with:
#               </div>
#             </div>
#           </div>
#         </div>
...
# Actually let's just find the exact div end.
if cross_end == -1:
    cross_end = html.find('</div>\\n        </div>\\n\\n        <!-- ==================== TAB 2', cross_start)
if cross_end == -1:
    cross_end = html.find('<div class="xl:col-span-3 space-y-3.5">', cross_start)
    if cross_end != -1:
        # Step back to the closing div of the previous column
        cross_end = html.rfind('</div>', 0, cross_end)
        cross_end = html.rfind('</div>', 0, cross_end)
        cross_end = html.rfind('</div>', 0, cross_end)
...
if box2_start != -1:
    # Remove up to the closing divs of section-eb
...
# Replace OSINT hardcoded logic
osint_code_start = html.find('// Mockup OSINT 360 values')
osint_code_end = html.find("const flagsList = document.getElementById('eb-flags-list');")
if osint_code_start != -1 and osint_code_end != -1:
    new_osint_js = '''// Dynamic OSINT & Cross-sell
        ...
        '''
    html = html[:osint_code_start] + new_osint_js + html[osint_code_end:]
